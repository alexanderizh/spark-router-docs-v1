...
<Callout type="info" title="Node.js 环境安装步骤">
  - 打开浏览器访问 https://nodejs.org/ - 点击 "LTS"
  版本进行下载（推荐长期支持版本）- 下载完成后双击 .msi 文件 -
  按照安装向导完成安装，保持默认设置即可
</Callout>
...
<Callout type="info" title="下载并安装 Git for Windows">
  - 访问 https://git-scm.com/downloads/win - 点击 "Download for Windows"
  下载安装包 - 运行下载的 .exe 安装文件 - 在安装过程中保持默认设置，直接点击
  "Next" 完成安装
</Callout>
...
<Callout type="info" title="安装 Claude Code">
  打开 PowerShell，运行以下命令： ` npm install -g @anthropic-ai/claude-code `
...
<Callout type="info" title="验证 Claude Code 安装">
  安装完成后，输入以下命令检查是否安装成功： ` claude --version `
...
<Callout type="info" title="启动 Claude Code">
  打开 PowerShell，直接启动 Claude Code: ` # 启动 Claude Code claude `
  在特定项目中使用: ` # 进入你的项目目录 cd C:\path\to\your\project ` ` # 启动
  Claude Code claude `
</Callout>
...
<Callout type="info" title="选择模型">
  输入命令： ` /model ` Enter进入，选择模型，通常使用默认设置即可。
</Callout>
...
<Callout type="info" title="安装 Claude Code">
  打开终端，运行以下命令： ` curl -fsSL https://claude.ai/install.sh | bash `
  可选项：出现提示后运行提供的命令 ` echo 'export PATH="$HOME/.local/bin:$PATH"'
...
<Callout type="info" title="启动 Claude Code">
  直接启动 Claude Code: ` # 启动 Claude Code claude ` 在特定项目中使用: ` #
...
<Callout type="info" title="选择模型 (可选)">
  输入命令： ` /model ` Enter进入，选择官方模型选择，通常使用默认模型即可
</Callout>
...
<Callout type="info" title="安装 Claude Code">
  打开终端，运行以下命令： ` curl -fsSL https://claude.ai/install.sh | bash `
  如果遇到权限问题，可以使用 sudo： ` sudo curl -fsSL
...
<Callout type="info" title="环境变量不生效" collapsible>
  检查以下几点：
</Callout>
