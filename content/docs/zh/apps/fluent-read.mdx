...
### 从 Spark Router 控制台导入配置（推荐）

当浏览器安装了流畅阅读插件后，打开 Spark Router 控制台->令牌管理页面会弹出添加流畅阅读的提示
...
确认导入后在流畅阅读中的Spark Router配置便会启用
...
| ---------- | -------------------------- |
| 翻译服务   | Spark Router                     |
| 访问令牌   | Spark Router 密钥                |
| Spark Router接口 | Spark Router部署地址（不带/v1）  |
| 模型       | 列表中选择，或者自定义模型 |
| 自定义模型 | 模型名称                   |
