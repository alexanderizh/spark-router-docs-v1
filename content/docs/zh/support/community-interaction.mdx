---
title: 社区互动
---

欢迎加入 Spark Router 开源社区！在这里，你可以与其他开发者交流经验，分享想法。

## 📜 群规则

为了维护良好的社区氛围，请遵守以下规则：

<Callout type="warn" title="重要提示">
  违反以下规则可能会被移出群聊，请各位成员仔细阅读并遵守。
</Callout>

1. 本群管理和群主没有任何义务为您提供任何技术支持，如有问题请提交 [issue](/zh/docs/support/feedback-issues)
2. 群聊禁止发布任何账号出售、求购相关信息，群昵称禁止出现任何购买引导、求购相关信息
3. 本群不出售任何api产品，请勿听信购买任何人的api产品（包括管理员的）
4. Spark Router 采用 GNU AGPLv3 开源协议，只要遵守开源协议即可免费使用。详情请查看[项目介绍](/zh/docs/guide/wiki/basic-concepts/project-introduction)
5. 必须在遵循相关法律法规的条件下使用本项目，禁止讨论任何非法话题

<Callout title="温馨提示">
  如果您有任何功能建议或bug反馈，建议直接在GitHub上提交issue，这样可以更好地追踪和解决问题。
</Callout>

## 🤝 加入我们的QQ交流群

<Callout type="info" title="加入前请完成问卷">
  请认真阅读上述群规则后，完成以下问卷验证。只有全部答对才能显示QQ群信息。
</Callout>

<QQGroupQuiz locale="zh" />
