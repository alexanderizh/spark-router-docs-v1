...
<Callout type="info" title="Official Documentation">
  [Reasoning Model
  (deepseek-reasoner)](https://api-docs.deepseek.com/en/guides/reasoning_model)
</Callout>
...
<Callout type="warn" title="Note">
  ...
  return a 400 error. Therefore, please remove the reasoning_content field from
  the API response before making the next API request, as shown in the usage
  example below.
</Callout>
