...
This document details the Docker Compose configuration options for Spark Router, applicable to various deployment scenarios.

## Basic Configuration Structure

The Docker Compose configuration file `docker-compose.yml` defines how the Spark Router service and its dependent services (such as MySQL, Redis) are deployed.
...
For testing purposes, you can use the following simplified version, which only includes the Spark Router service itself:
...
### Spark Router Service Configuration
...
| `environment`    | Environment variable settings, used to configure Spark Router behavior              |
...
Spark Router supports various environment variable configurations. Here are some commonly used ones:
