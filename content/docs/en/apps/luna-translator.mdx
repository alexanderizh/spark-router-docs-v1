...
<Callout type="info" title="Chat Settings Options">
  In the Spark Router console's System Settings -> Chat Settings, you can add the
...
## Integrating Spark Router into LunaTranslator

LunaTranslator supports integrating with locally deployed Spark Router and third-party Spark Router services built using Spark Router.

### One-Click Configuration

1.  In the Spark Router console's `System Settings` -> `Chat Settings`, add the following shortcut option:
...
2.  In the **`Spark Router`** -> `Console` -> `Token Management` tab, select the token to be used in LunaTranslator, click the dropdown option next to the chat button, select `LunaTranslator`, and it will jump to LunaTranslator and automatically configure the API address and API Key.
...
4.  Click the refresh button next to the **model** dropdown box to get the Spark Router platform's model list, select or enter the model name, then click OK to save.
...
1.  In the **`Spark Router`** -> `Console` -> `Token Management` tab, obtain the API Key.
...
5.  Click the refresh button next to the **model** dropdown box to get the Spark Router platform's model list, select or enter the model name, then click OK to save.

    ![设置 API3](/assets/luna_translator/setting_api3.png)

6.  Click the toggle button next to **Spark Router** to enable the interface and start using it.
