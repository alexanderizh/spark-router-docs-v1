...
Ratio settings are a core configuration of the Spark Router billing system, allowing flexible control over the billing standards for various models and user groups by setting different ratios.

## Ratio System Overview

Spark Router uses a three-tier ratio system to calculate user quota consumption:
...
In the Spark Router system, ratios are key parameters for calculating quota consumption. Quota is an internal billing unit of the system, and all API calls are ultimately converted into quota points for deduction.
...
#### Audio Models (Special handling, automatically processed internally by Spark Router)
...
Spark Router adopts a dual billing mechanism of pre-consumption and post-consumption:
