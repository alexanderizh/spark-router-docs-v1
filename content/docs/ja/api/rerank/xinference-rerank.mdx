...
<Callout type="warn" title="重要なお知らせ">
  New
  APIでは、Xinferenceのrerank応答構造はJinaのrerank応答構造としてフォーマットされ、Jinaのrerankと同じ方法で使用されます。**Difyなどのクライアントユーザーの方へ**：設定時には、Xinferenceではなく
  **Jina AI**
  をプロバイダータイプとして選択し、Xinferenceがサポートするモデル名を使用してください。
</Callout>
